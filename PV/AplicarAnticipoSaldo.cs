...
namespace PV
{
    public partial class AplicarAnticipoSaldo : Form
    {
        ...
        public AplicarAnticipoSaldo(string importe, string Matricula, string Alumno, string anticipo)
        {
            InitializeComponent();
        ...
        private void dgvPagosPendientes_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            dgvPagosPendientes.Rows[e.RowIndex].Cells[5].ReadOnly = true;

            decimal Importe = 0.00M;
            decimal Abono = 0.00M;
            decimal Saldo = 0.00M;

            if (dgvPagosPendientes.Rows[e.RowIndex].Cells[3].Value != null)
            {
                Importe = Convert.ToDecimal(dgvPagosPendientes.Rows[e.RowIndex].Cells[3].Value.ToString());
            }
            if (dgvPagosPendientes.Rows[e.RowIndex].Cells[5].Value != null)
            {
                Abono = Convert.ToDecimal(dgvPagosPendientes.Rows[e.RowIndex].Cells[5].Value.ToString());

            }
            else
            {
                dgvPagosPendientes.Rows[e.RowIndex].Cells[5].Value = 0;
            }
            ...
            dgvPagosPendientes.Rows[e.RowIndex].Cells[6].Value = saldo.ToString();
            ...
            decimal abono = Convert.ToDecimal(dgvPagosPendientes.Rows[e.RowIndex].Cells[5].Value);
            dgvPagosPendientes.Rows[e.RowIndex].Cells[5].Value = abono.ToString("N", formato);
        }
        ...
        private void dgvPagosPendientes_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (this.dgvPagosPendientes.Columns[e.ColumnIndex].Name == "MasAbono")
            {
                ...
                dgvPagosPendientes.Rows[e.RowIndex].Cells[5].ReadOnly = false;
                dgvPagosPendientes.Rows[e.RowIndex].Cells[5].Selected = true;
                dgvPagosPendientes.BeginEdit(true);
            }
            else
            {
                dgvPagosPendientes.Rows[e.RowIndex].Cells[5].ReadOnly = true;
                dgvPagosPendientes.Rows[e.RowIndex].Cells[5].Selected = false;
            ...
            else
            {
                if (MessageBox.Show("¿Finalizar aplicacion de Anticipo?", "Aplicar Anticipo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    foreach (DataGridViewRow row in dgvPagosPendientes.Rows)
                    {
                        if (Convert.ToDecimal(row.Cells["Abono"].Value.ToString()) > 0)
                        {
                            c.ActualizarRecibo2(row.Cells["FolioDocumento"].Value.ToString(), Convert.ToDecimal(row.Cells["Saldo"].Value.ToString()));
                            c.InsertarCobro(row.Cells["FolioDocumento"].Value.ToString(), txtMatricula.Text, dtpFecha.Text, Convert.ToDecimal(row.Cells["Abono"].Value.ToString()), txtFolioGeneral.Text);
